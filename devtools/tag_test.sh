...
cat src/fst/morphology/clitics.lexc src/fst/morphology/compounding.lexc  src/fst/morphology/affixes/*lexc src/fst/morphology/stems/*lexc src/fst/morphology/generated_files/*lexc |cut -d '!' -f1 |grep ' ;' | cut -d ':' -f1 |tr -s ' ' |sed 's/^ //' | cut -d ' ' -f1 |sed 's/+/¢+/g' |sed 's/@/¢@/g'|tr '¢' '\n' | tr '#"%' '\n'| egrep '(\+|@)' |sort -u | egrep -v '^(\+|\+\/\-|\+Cmp\-|\@0)$' > lexctags
